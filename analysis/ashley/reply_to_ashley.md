...
5. The caveat. While looking into this we found three inherited defects in the 2022 FUN code: two root uptake cost parameters were swapped (the same problem later fixed in CTSM, issue 2120), a PFT index typo, and a hardcoded fixation cost near zero. All three push carbon away from mycorrhizae in the optimisation, so the numbers above are a lower bound. A corrected rerun of the full 1850 to 2010 simulation is running now, with the AM and EcM diagnostics wired in, and I expect the corrected layers within about a week. I would use those for the paper and will send them as soon as they are done. Everything is documented at github.com/braghiere/ELM-FUN-2022-corrigendum.
...
- ELM_FUNP_2022_mycorrhizal_C_layers_2001-2010.nc (provisional 2022 layers: mycorrhizal C, NPP before the acquisition cost, their ratio, AVAILC, COST_NACTIVE, COST_PACTIVE; caveats in the file attributes)
