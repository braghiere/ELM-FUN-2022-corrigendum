...
ax.set_title('a  Where the nutrient cost bites (control run, 1994–2005); outline = cells with > 50 % C4 grass',loc='left',fontsize=10,color=T1)
# (b) scatter
...
for name,m,col in groups:
    mm=ok&m; ax.scatter(sminn[mm],price[mm],s=22,c=col,edgecolors=SURF,linewidths=0.6,label=f'{name} ({mm.sum()} cells)',zorder=3 if col!=GRAY else 2)
...
ax.set_title('b  Carbon price of nitrogen falls with soil mineral N; C4-grass cells sit in the poor corner',loc='left',fontsize=10,color=T1)
# (c,d) bars
