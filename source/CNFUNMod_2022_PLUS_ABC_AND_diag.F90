...
! --------------------ACTIVE UPTAKE POX UPTAKE LIMIT------------------------!  
     ...
     ! free P goes straight to the ppool, not throught Puptake...
      ...
      ! --- AM/EcM split of mycorrhizal N-uptake C (diagnostic only, exact closure; wired 2026-09) ---
      npp_Necm(p) = ( npp_active_no3_acc(p,ecm_step) + npp_active_nh4_acc(p,ecm_step) &
                    + npp_active_no3_retrans(p,ecm_step) + npp_active_nh4_retrans(p,ecm_step) )/dt
      npp_Nam(p)  = npp_Nactive(p) - npp_Necm(p)
      ...
      if(use_funp)then
        ...
        ! --- AM/EcM split of mycorrhizal P-uptake C (diagnostic only, exact closure; wired 2026-09) ---
      
        npp_Pecm(p) = ( npp_active_pox_acc(p,ecm_step) + npp_active_pox_retrans(p,ecm_step) )/dt
      
        npp_Pam(p)  = npp_Pactive(p) - npp_Pecm(p)
...
!--------------------------------------------------------------------------
  ...
  if (fixer == 1 .and. crootfr > 1.e-6_r8) then
     ...
     ! Defect-C fix (2026-09-16): hardcoded s_fix=-30 / b=-0.27 override disabled; paramfile-based cost above now executes
