  ...
  subroutine ColCBalanceCheck(bounds, &
    ...
    associate(                                                                           &
      ...
      do fc = 1,num_soilc
         ...
         ! CBAL-TEST 2026-09-19 (corrigendum): threshold 1e-7 -> 1e-4 gC/m2/step, so only real leaks are reported
         if (abs(col_errcb(c)) > 1e-4_r8) then
            err_found = .true.
            err_index = c
         end if
         ! CBAL-TEST: always report one reference column (Amazon, ~2.8S 300E, natural-veg column) every step, FUN on or off
         if (abs(grc_pp%latdeg(col_pp%gridcell(c)) + 2.8421_r8) < 0.5_r8 .and. abs(grc_pp%londeg(col_pp%gridcell(c)) - 300._r8) < 1.0_r8 .and. col_pp%itype(c) == 1) then
            write(iulog,'(a,i8,7(1x,e17.9))') 'CBALREF ', c, col_cinputs(c)*dt, col_coutputs(c)*dt, er(c)*dt, ar(c)*dt, col_fire_closs(c)*dt, col_endcb(c)-col_begcb(c), col_errcb(c)
         end if
         if(.not.use_fun)then
      ...
      if (.true. ) then   ! CBAL-TEST: was (.not. use_fun); report with FUN on as well
